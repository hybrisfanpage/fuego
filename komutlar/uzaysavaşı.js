...
exports.run = async (client, message, args) => {
  ...
  if (opponent.id === message.author.id) return message.reply('Kendin ile Uzay Savaşı oynayamassın!');
		if (this.fighting.has(message.channel.id)) return message.reply('Kanal başına sadece bir Uzay Savaşı meydana gelebilir.');
		this.fighting.add(message.channel.id);
		try {
			if (!opponent.bot) {
                await message.channel.send(`${opponent}, Uzay Savaşı isteği geldi. Uzay Savaşı'nı kabul ediyor musun? (\`evet\` veya \`hayir\` olarak cevap veriniz.)`);
				const verification = await verify(message.channel, opponent);
				if (!verification) {
					this.fighting.delete(message.channel.id);
					return message.channel.send(`Uzay Savaşı kabul edilmedi...`);
				}
			}
			let userHP = 500;
			let oppoHP = 500;
			...
			while (userHP > 0 && oppoHP > 0) {
				...
				if (choice === 'lazer') {
					...
					if (!miss) {
						...
						await message.channel.send(`${user}, TCG Pardus Uzay Gemi'si imdadına yetişti ve onun desteğiyle  **${damage}**  hasar vurdun!!`);
						dealDamage(damage);
					} else {
					...
					await message.channel.send(`${user},Uzay Gemisinin tüm enerjisini Işık hızı motoruna enerjiyi yönlendirip savaştan kaçtı`);
					forfeit();
					break;
				} else {
...
exports.conf = {
  enabled: true,
  guildOnly: false,
  aliases: ['us', 'uzay-savası', 'uzaysavaşları'],
