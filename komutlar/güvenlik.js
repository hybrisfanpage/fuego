...
.setAuthor("Fuego Bot", client.user.avatarURL)
.setTitle("Güvenlik Sistemi")
.setDescription("Tek Yapman Gereken **%güvenlik #kanal** Yazmak! Kapatmak İçin **%güvenlik sıfırla**")
...
message.channel.send(mbed)
  ...
  if (args[0] === "sıfırla" || args[0] === "kapat") {
    if(!logkanal) return message.channel.send(`**:fire: Güvenliği kapatmak için \`güvenlik kanalının\` seçili olması lazım örnek kullanım: \`%güvenlik #kanal\`**`);
    
   db.delete(`guvenlik${message.guild.id}`)
   message.channel.send(`**:fire: Güvenlik başarıyla kapatıldı.**`);
...
message.channel.send(`**:fire: Güvenlik başarıyla ${logk} olarak ayarlandı**`);
