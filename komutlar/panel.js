...
exports.run = async function(client, msg, args) {
  let ayar = args[0];
  if(!ayar) {
    return msg.reply("**:fire: Paneli Açmak İçin \`%panel aç\` Kapatmak İçin \`%panel kapat\` Yazınız**")
  } else if(ayar == "aç") {
    let sistem = await db.fetch(`panell_${msg.guild.id}`);
    if(sistem || sistem == "açık") {
      msg.reply("**:fire: Sistem zaten açık durumda!")
    } else {
      db.set(`panell_${msg.guild.id}`, "açık");
      msg.reply("**:fire: Sistem açık hâle getirildi!**")
    }
  } else if(ayar == "kapat") {
    let sistem = await db.fetch(`panell_${msg.guild.id}`);
    if(!sistem) {
      msg.reply("**:fire: Sistem zaten kapalı durumda!**")
    } else {
      ...
      msg.reply("**:fire: Sistem kapalı hâle getirildi!**");
